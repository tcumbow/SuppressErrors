			...
			if SV.motd == 1 then
				...
				for i = 1, GetNumGuilds() do
					...
					if savedMotD ~= currentMotD then
						...
						local message = zo_strformat("<<X:1>> |cFFFFFF<<2>>\n<<3>>", allianceIcon, guildName, currentMotD)
						SafePrint(message)
					end
...
local function BuildSettingsMenu()
	...
	panel.logo:SetTexture("SuppressErrors/Art/noty.dds")
...
local function OnAddonLoaded(event, name)
	if name == ADDON_NAME then
		...
		SV = ZO_SavedVars:NewAccountWide("SUPPRESS_ERRORS_VARS", 1, defaults)
